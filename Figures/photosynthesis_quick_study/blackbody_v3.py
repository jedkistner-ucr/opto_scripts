...
ir = 50
uv = 100
visrange = int(600 - ir - uv)
vis_ = int(visrange * 1.27)

x1 = np.zeros(rad.shape)
clr = color_meline(colormap, vis_)
for i in range(visrange):
    ax[0].fill_between( w[uv + i:uv+i+1], x1[i:i+1], rad[uv+i:uv+i+1], color = clr[vis_-visrange+i])
#left
clr = color_meline(colormap1, uv*2)
for i in range(uv):
    ax[0].fill_between( w[  i:+i+1], x1[i:i+1], rad[i:+i+1], color = clr[uv+i])
#right
clr = color_meline(colormap2, ir*2)
for i in range(ir):
    ax[0].fill_between( w[ 600-ir + i:600-ir+i+1], x1[i:i+1], rad[600-ir+i:600-ir+i+1], color = clr[i])

# ax[0].fill_between( w, x1, rad, color = clr[0])
...
if save:
    pl.savefig("C:/QMO/generated plots/photosyn/" + "blackbody_3" +".png", dpi=400)
