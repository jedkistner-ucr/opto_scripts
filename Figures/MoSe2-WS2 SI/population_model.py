...
N = np.zeros(t.size)
pci = np.zeros(t.size)
pci_tot = np.zeros(t.size)
eh = np.zeros(t.size)
N[0] = initialpop

for i in range(timesteps-1):
    a, b = dn_dt(alpha, tau, N[i])
...
ax[0].set_xlabel("Time (arb. seconds)")
ax[0].set_ylabel("Charge carriers in heterojunction (count)")
...
pl.savefig("C:/QMO/generated plots/electron_hole/" + "population_model" +".png")
