...
def main(savemod = 0, plotgline = 0):
    ...
    # setgate = np.searchsorted(yval, -6.44)
    # data = data[setgate, :,:]
    # xval = cval
    ...
    axs, figs = customPlot(3, 3, sidebar = False)
    ax = []
    ...
    if fit:
        ptrim = np.searchsorted(xval, trimval)
    ...
    for i in range(len(gatecuts)):
        ...
        if plotgline[i] == 1:
            ax[0].scatter(x, y, s = width[i], c = clrgate[i], alpha = alpha[i])
        else:
            ax[0].scatter(x, y, s = width[i], c = clrgate[i], alpha = 0)
        ...
        if fit:
            try:
                ...
                ax[0].plot(x_, fitfunc(x_, *par), linewidth = width1[i], linestyle = style1[i], c = speccolor1[i])
            except RuntimeError:
                None
