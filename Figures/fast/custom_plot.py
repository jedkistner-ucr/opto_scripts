...
def main(savemod = 0, plotgline = 0):
    ...
    # name1 = "CPS_2021_03_25_29"
    # name = "CPS_2022_02_01_16"
    # name1 = "CPS_2022_02_01_6"
    ...
    # setgate = np.searchsorted(yval, -5.2)
    # data = data[:,:,18]
    # data = data[setgate,:,:]
    ...
    # gatecuts = np.linspace(0, .15, 40)
    gatecuts = np.asarray([2])
    # gatecutsindex = [0, 3, 6, 9, 12, 15, 18]
    # gatecutsindex = range(0, 40)
    gatecutsindex = []
    ...
    width = np.ones(gatecuts.shape)
    alpha = np.ones(gatecuts.shape)
    ...
    for i in range(len(gatecuts)):
        if direrction == "x":
            if gatecutsindex == []:
                if cube:
                    gateindex = np.searchsorted(cval, gatecuts[i])
                else:
    ...
    if double or addsecond:
        for i in range(len(gatecuts1)):
            if plotgline1[i] == 1:
                ...
                if addsecond:
                    ax[0].plot(x, y, linewidth = width1[i], c = speccolor1[i], linestyle = style1[i], alpha = alpha1[i])
                    # xx = np.linspace(np.max(xval1), np.min(xval), 10)
                    # yy = np.zeros(xx.shape)
                    # ax[0].plot(xx, yy, linewidth = width1[i], c = speccolor1[i], linestyle = style1[i], alpha = alpha1[i])
