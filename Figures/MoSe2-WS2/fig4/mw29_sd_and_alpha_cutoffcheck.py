...
path = "C:/QMO/Built"
savename = "MW29_sd_c_smoothcheck"
...
zerofit = True
powercutoff = [.1, .11, .14, .15, .16, .17, .18, .19, .2, .21, .22, .23, .24]
powercutoff = [.14, .15, .16, .17, .18, .19, .2, .21, .22, .23, .24]
...
clr = color_meline('plasma', len(powercutoff))
alpha = []
for q in range(len(powercutoff)):
    powerindex = np.searchsorted(cval, powercutoff[q])
    a = np.zeros(xval.shape)
    ...
    ax[0].plot(xval, a, linewidth = linewidth, c = clr[q])
    alpha.append(a)
...
ax[0].plot(xval, a, linewidth = linewidth, c = 'k')
