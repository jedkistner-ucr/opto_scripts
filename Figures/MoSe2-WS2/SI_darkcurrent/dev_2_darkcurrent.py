...
for i in range(len(hlines)):
    ax[plotno].axhline(hlines[i], linestyle = "--", c = hclr[i], linewidth = 2, alpha = 1)
for i in range(len(vlines)):
    ax[plotno].axvline(vlines[i], linestyle = ":", c = vclr[i], linewidth = 2, alpha = 1)
...
x = xval
for i in range(len(hlines)):
    gateindex = np.searchsorted(yval, hlines[i])
    y = data[gateindex,:]

    ax[plotno].plot(x, y, linewidth = linewidth, c = hclr[i])
...
x = yval
for i in range(len(vlines)):
    sdindex = np.searchsorted(xval, vlines[i])
    y = data[:,sdindex]

    ax[plotno].plot(x, y, linewidth = linewidth, c = vclr[i])
...
plotno = 4
ylabel = "dlog ($\it{I}$) / d$\itV_{g}$ (nA/V)"
xlabel = "$\itV_{g}$ (V)"
plabel = ""
