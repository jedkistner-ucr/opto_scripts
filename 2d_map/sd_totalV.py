...
axs, axcbs, axscbs, figs = makeaxes(1 + len(xindex), cbar = True, scalebar=True, scale = 1)
ax = []
axsc = []
    ...
    if xi > 0:
        for i in range(clen):
            # if i > 0 and i < ylen - 1:
                ...
                x = np.zeros(ylen - xi)
                y = np.zeros(ylen - xi)
                for p in range(ylen - xi):
                    y[p] = data[xi + p,p,i]
                    x[p] = xval[p] + yval[xi + p]

                ax[1+q].plot(x, y, linewidth = 1.5, c = clrsd[i])

        ax[0].plot(xval, xval - yval[xi], linewidth = 2, linestyle = ":", c = 'k')
        ...
        for i in range(clen):
            # if i > 0 and i < ylen - 1:
                ...
                x = np.zeros(xlen - xi)
                y = np.zeros(xlen - xi)
                for p in range(xlen - xi):
                    y[p] = data[p,xi + p,i]
                    x[p] = xval[xi +p] + yval[p]

                ax[1+q].plot(x, y, linewidth = 1.5, c = clrsd[i])
        
        ax[0].plot(xval - xval[xi], xval, linewidth = 2, linestyle = ":", c = 'k')
        ...
        ax[0].plot(xval, xval, linewidth = 2, linestyle = ":", c = 'k')
...
for q in range(len(xindex)):
    ax[1+q].set_ylim( ymin2 - margin2 , ymax2 + margin2 )
...
if xlimits == [0]:
    for i in range(len(xindex)):
        ax[1+i].set_xlim(xmin1, xmax1)
else:
    for i in range(len(xindex)):
        ax[i].set_xlim(xlimits[0], xlimits[1])

if ylimits != [0]:
    for q in range(len(xindex)):
        ax[1+q].set_ylim(ylimits[0], ylimits[1])
...
today = date.today()
savename = name + "_sd_totalV_" + str(today)
